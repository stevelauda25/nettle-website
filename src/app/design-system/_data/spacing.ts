...
export const spacingExceptions = [
  ...
  "Features, Business Lines and Testimonial show Previous/Next controls from md (768px) upward, aligned to the heading row's right edge, and hide them only on mobile. The controls reuse the shared content and related-gap spacing roles with no mobile header reservation. Headings remain left-aligned and balanced. Shared carousel sizing exposes two grid gutters (32px) of the next card on phones, while retaining desktop card spans, native swipe/snap and focusable keyboard navigation. Button padding and footer 24px link-to-link gaps remain unchanged.",
  ...
  "Video desktop scroll geometry and its zero-padding override remain interaction-specific. Mobile uses equal media padding above and below its poster inside the sticky stage, with 20svh of travel and 95% to 100% scaling. Pin the stage just below the approximately 5.75rem mobile header, not at the viewport midpoint: midpoint pinning creates excess whitespace on tall phones. Keep the poster centered within the compact stage. Reduced-motion/unsupported browsers retain the same symmetric padding in natural-height flow. See video-explainer.module.css.",
  "Mobile Hero minimum height accounts for the approximately 5.75rem sticky header, so header + Hero fill the first viewport without adding a second header allowance. The announcement bar is 40px tall after a user-requested 2px reduction on each side; its link retains a 44px hit area. Hero content remains centered with symmetric opening-end padding and can grow for enlarged text. From md it remains content-driven with opening-start/top and opening-end/bottom padding. Role-card grid rows, CTA minimum height and desktop testimonial artwork rows are unchanged.",
  "Challenge uses one viewport-minus-header sticky stage and section-local scroll travel (210svh mobile, 280svh from md), with gradual, non-overlapping text transitions on the compact track. Its stage can grow with text; a following spacer supplies the travel without clipping content. Reduced motion, unsupported timelines and viewports shorter than 30rem show all three statements in normal flow with section-standard padding/gaps. Timeline and cover-crop geometry live in challenge-today.module.css; these are interaction exceptions, not changes to the shared spacing scale. Mobile artwork has no supplied Figma frames and remains an inferred crop requiring manual QA.",
  "Security mobile cards use a 400px minimum instead of 460px. Their artwork region crops from 370 × 299 to 370 × 239 and linework shifts upward 60 artwork-coordinate pixels; badges keep their original position. Copy remains in normal flow with the shared card padding and can grow the cards. From md, the 460px minimum and original artwork composition are unchanged. This is a user-requested mobile composition adjustment, not a shared spacing-token change.",
